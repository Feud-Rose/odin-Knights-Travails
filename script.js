...
function knightMovement(start, end){
    ...
    const movementRange = [[-1,2],[1,2],[-2,1],[2,1],[-2,-1],[2,-1],[-1,-2],[1,-2]]   
    console.log(end)
    //Check start and end
    ...
    if (JSON.stringify(start) === JSON.stringify(end)) return console.log("Already There")
    visited = new Set()
    ...
    while(queue.length !=0){
        ...
        console.log(current)
        console.log(queue)
        if(current.parent) parent.child = (current.position[0],current.position[1])
       ...
       for(let i = 0; i < movementRange.length; i++){
            ...
            if(JSON.stringify(nodeTemp.position) === JSON.stringify(end)) return nodeTemp
           
            ///Only Queue up new squares
            let check = visited.has(`${nodeTemp.position[0]}_${nodeTemp.position[1]}`)
            if(!check){
...
function buildPath(node,path=[]){
    ...
    if(node.parent === null){
        ...
        return console.log(`Reached in ${path.length} moves ${JSON.stringify(path.reverse())}`)
    } 
